...
export default function FullWidthGrid(props) {
  ...
  const [presentingMode, setPresentingMode] = React.useState(false);
    ...
    return data && data[0] ? (
      data.map((value) => {
        return !value || typeof value === "string" || value[0] ? (
          <div className={presentingMode?classes.box + " box":''}>- {value ? value : ""}</div>
        ) : (
          <>
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={() => setPresentingMode(!presentingMode)}
              />
            }
